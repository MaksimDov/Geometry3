...
int peresech(double x1, double y1, double x2, double y2, double x3, double y3,
	...
	if (!f1 && !f2) {
		xIntersection =
		    ...
		    (slope1 - slope2);  // Iaoiaei ?aoaiea nenoaiu o?aaiaiee
		yIntersection = slope1 * xIntersection + yIntercept1;
		if (getCos(x1 - xIntersection, y1 - yIntersection,
			   x2 - xIntersection, y2 - yIntersection) <= 0 &&
		    getCos(x3 - xIntersection, y3 - yIntersection,
			   x4 - xIntersection, y4 - yIntersection) <= 0)
			...
			if (projectionsIntersect(x1, x2, x3, x4))
				...
				/* cout << "On the same line, but don't
				 * intersect";*/
				fact = false;
		} else if (yIntercept1 != yIntercept2 &&
